 ...
 */
public class BlockingBroadcastTransmitter
{
     ...
     */
    public BlockingBroadcastTransmitter(final AtomicBuffer buffer)
    {
        this.buffer = buffer;
    ...
    public void addSubscriber(BlockingBroadcastReceiver receiver) {
    ...
    public void transmit(final int msgTypeId, final DirectBuffer srcBuffer, final int srcIndex, final int length)
    {
        ...
        while ((currentTail + recordLength - minimumReceiverCursor()) > capacity()) {
//            System.out.println("BlockingBroadcastTransmitter: Back pressured");
//            try { Thread.sleep(1000); } catch (InterruptedException exception) { exception.printStackTrace(); }
    ...
    private long minimumReceiverCursor() {
        if (receivers.isEmpty()) return 0L;
        long cursor = Long.MAX_VALUE;
        for (BlockingBroadcastReceiver receiver : receivers) cursor = Math.min(cursor, receiver.cursor());
        return cursor;
    }
}