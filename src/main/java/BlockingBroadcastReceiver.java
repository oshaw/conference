/*
 * Modified by Oscar Shaw (https://www.github.com/oshaw).
 */
...
/**
 * Receive messages broadcast from a {@link BlockingBroadcastTransmitter} via an underlying buffer. Receivers can join
 ...
 */
public class BlockingBroadcastReceiver
{
     ...
     *
     * @param transmitter containing the buffer via which messages will be exchanged.
     ...
     */
    public BlockingBroadcastReceiver(final BlockingBroadcastTransmitter transmitter)
    {
        this.buffer = transmitter.buffer();
        ...
        cursor = new AtomicLong(buffer.getLongVolatile(latestCounterIndex));
        nextRecord = cursor.get();
        recordOffset = (int)cursor.get() & (capacity - 1);
        transmitter.addSubscriber(this);
    }
    ...
    public boolean validate()
    {
        ...
        return validate(cursor.get(), buffer, capacity);
    }
